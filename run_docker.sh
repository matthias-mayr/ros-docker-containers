...
if [[ "$1" == "melodic" ]]; then
	...
	if [[ "$2" == "nvidia" ]]; then
    	...
    	export DISPLAY=:0
    	xhost + 
     	docker run -it --user=$( id -u $USER ):$( id -g  $USER ) --ipc=host --device=/dev/dri:/dev/dri --net=host --privileged --env="DISPLAY=$DISPLAY" --workdir="/home/$USER" --volume="/home/$USER:/home/$USER" --volume="/etc/group:/etc/group:ro" --volume="/etc/passwd:/etc/passwd:ro" --volume="/etc/shadow:/etc/shadow:ro" --volume="/etc/sudoers.d:/etc/sudoers.d:ro" --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" ros:kinetic-desktop-full-9-graphics-intel
    fi
fi
